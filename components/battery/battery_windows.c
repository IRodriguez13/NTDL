...
int battery_get_metrics(BatteryMetrics *metrics) {
    ...
    if (power_status.BatteryFlag == 128) { // No system battery
    ...
    // Nivel de carga
    if (power_status.BatteryLifePercent != 255) { // 255 = unknown
    ...
    // Estado de carga
    if (power_status.ACLineStatus == 1) { // AC power online
        if (power_status.BatteryFlag & 8) { // Charging
    ...
    // Tiempo restante
    if (power_status.BatteryLifeTime != 0xFFFFFFFF) { // 0xFFFFFFFF = unknown
    ...
    if (power_status.BatteryFlag & 1) { // High
        strncpy(metrics->health, "Good", sizeof(metrics->health) - 1);
    } else if (power_status.BatteryFlag & 2) { // Low
        strncpy(metrics->health, "Low", sizeof(metrics->health) - 1);
    } else if (power_status.BatteryFlag & 4) { // Critical
...
float battery_get_charge_percent(void) {
    ...
    if (power_status.BatteryLifePercent == 255) { // Unknown
        return -1.0f;
    }
...
int battery_get_time_remaining_minutes(void) {
    ...
    if (power_status.BatteryLifeTime == 0xFFFFFFFF) { // Unknown
        return -1;
    }
...
int battery_get_status(char *buffer, size_t size) {
    ...
    if (power_status.BatteryFlag == 128) { // No battery
        strncpy(buffer, "No Battery", size - 1);
    } else if (power_status.ACLineStatus == 1) { // AC power online
        if (power_status.BatteryFlag & 8) { // Charging
