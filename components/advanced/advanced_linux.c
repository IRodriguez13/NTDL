...
static float read_hwmon_sensor(const char *pattern, const char *sensor_name) {
    glob_t glob_result;
    ...
    if (fp) {
        ...
        while (fgets(line, sizeof(line), fp)) {
            ...
            if (strstr(line, "Memory Device")) {
                current_slot++;
                if (current_slot >= MAX_MEMORY_SLOTS) break;
            }
