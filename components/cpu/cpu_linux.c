            ...
            for (size_t j = 0; j < glob_result.gl_pathc; j++) {
                ...
                if (f) {
                    ...
                    if (fscanf(f, "%d", &temp_millicelsius) == 1) {
                        ...
                        if (temp_celsius > 0 && temp_celsius < 150) { // Rango razonable
...
int cpu_get_metrics(CpuMetrics *metrics) {
    ...
    metrics->num_cores_info = (metrics->logical_cores < MAX_CORES) ? 
                              metrics->logical_cores : MAX_CORES;
...
int cpu_get_core_info(int core_id, CoreInfo *core_info) {
    if (!core_info || core_id < 0) return -1;
