...
static int get_disk_smart_via_smartctl(const char *device, DiskInfo *info) {
    ...
    if (fp && fgets(line, sizeof(line), fp)) {
        ...
        if (model) {
            model++; // Saltar ':'
            while (*model == ' ') model++; // Saltar espacios
...
static int scan_mounted_disks(void) {
    ...
    while (fgets(line, sizeof(line), f) && disk_count < MAX_DISKS) {
        ...
        if (sscanf(line, "%255s %255s %63s", device, mount_point, filesystem) == 3) {
            ...
            if (strstr(device, "loop") != NULL) continue; // Saltar loop devices
            if (strstr(mount_point, "/snap/") != NULL) continue; // Saltar snap mounts
            
            // Saltar sistemas de archivos virtuales
            if (strcmp(filesystem, "proc") == 0 || strcmp(filesystem, "sysfs") == 0 ||
                strcmp(filesystem, "devpts") == 0 || strcmp(filesystem, "tmpfs") == 0) {
                continue;
            }
            ...
            disk->device[sizeof(disk->device) - 1] = '\0';
            
            strncpy(disk->mount_point, mount_point, sizeof(disk->mount_point) - 1);
            disk->mount_point[sizeof(disk->mount_point) - 1] = '\0';
...
int disk_get_io_stats(const char *device_path, uint64_t *read_bytes, uint64_t *write_bytes,
    ...
    if (get_disk_io_stats_from_proc(device_path, &temp_info) == 0) {
        if (read_bytes) *read_bytes = temp_info.read_bytes;
        if (write_bytes) *write_bytes = temp_info.write_bytes;
        if (read_ops) *read_ops = temp_info.read_ops;
        if (write_ops) *write_ops = temp_info.write_ops;
        return 0;
    }
