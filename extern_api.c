...
const char *sysmon_get_os_name(void) {
    if (ensure_status_valid() != 0) return "Unknown";
...
int sysmon_get_cpu_cores(void) {
    if (ensure_status_valid() != 0) return -1;
...
float sysmon_get_cpu_usage_percent(void) {
    ...
    // Fallback al cache
    if (ensure_status_valid() != 0) return -1.0f;
...
const CoreInfo *sysmon_get_core_info(int core_id) {
    if (ensure_status_valid() != 0) return NULL;
    if (core_id < 0 || core_id >= g_system_status.cpu.num_cores_info) return NULL;
...
unsigned long sysmon_get_ram_total_kb(void) {
    if (ensure_status_valid() != 0) return 0;
...
const char *sysmon_get_gpu_model(void) {
    if (ensure_status_valid() != 0) return "Unknown";
    if (g_system_status.num_gpus == 0) return "No GPU";
...
unsigned long sysmon_get_gpu_memory_total_mb(void) {
    if (ensure_status_valid() != 0) return 0;
    if (g_system_status.num_gpus == 0) return 0;
...
float sysmon_get_gpu_usage_percent(void) {
    if (ensure_status_valid() != 0) return -1.0f;
    if (g_system_status.num_gpus == 0) return -1.0f;
...
int sysmon_get_gpu_fan_speed(void) {
    if (ensure_status_valid() != 0) return -1;
    if (g_system_status.num_gpus == 0) return -1;
...
const DiskInfo *sysmon_get_disk_info(int disk_id) {
    if (ensure_status_valid() != 0) return NULL;
    if (disk_id < 0 || disk_id >= g_system_status.num_disks) return NULL;
...
const NetworkInfo *sysmon_get_network_info(int interface_id) {
    if (ensure_status_valid() != 0) return NULL;
    if (interface_id < 0 || interface_id >= g_system_status.num_network_interfaces) return NULL;
...
const SensorInfo *sysmon_get_sensor_info(int sensor_id) {
    if (ensure_status_valid() != 0) return NULL;
    if (sensor_id < 0 || sensor_id >= g_system_status.num_sensors) return NULL;
